...
function MainPage() {
  ...
  return (
    <Container
      ...
      exit={{ opacity: 0 }}
    >
      <TopItems geolocationState={geolocationState} />
...
function TopItems(props) {
  return (
    <TopContainer>
      <div>검색</div>
      <div>top items</div>
      <Weather geolocationState={props.geolocationState} />
    </TopContainer>
  );
}
...
function Weather(props) {
  console.log(props.geolocationState);
  return (
    <div>
      {props.geolocationState === "success" ? (
        <p>실패임 ㅅㄱ</p>
      ) : (
        <p>성공임</p>
      )}
